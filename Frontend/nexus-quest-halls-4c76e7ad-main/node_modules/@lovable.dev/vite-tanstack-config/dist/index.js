...
function defineConfig(configOrOptions = {}) {
	return async (env) => {
		...
		if (command === "serve" && isSandbox && bundledDevRequested()) {
			...
			if (viteVersionSupportsBundledDev(viteVersion)) config = mergeConfig(config, { experimental: { bundledDev: true } });
			else logWarning(`Bundled dev requested (${BUNDLED_DEV_ENV_VAR}) but the installed vite (${viteVersion ?? "unresolved"}) is missing, prerelease, or below 8.1 — starting classic dev.`);
		}
		if (isSandbox) {
