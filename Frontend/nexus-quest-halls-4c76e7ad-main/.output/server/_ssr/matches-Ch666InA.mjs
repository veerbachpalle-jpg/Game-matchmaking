import { o as __toESM } from "../_runtime.mjs";
import { a as require_react, i as require_jsx_runtime, t as useQuery } from "../_libs/react+tanstack__react-query.mjs";
import { a as useAuth, r as api } from "./use-auth-xjOyXtJV.mjs";
import { _ as useNavigate, g as Link } from "../_libs/@tanstack/react-router+[...].mjs";
import { n as Alert, o as Panel, r as ArenaShell, s as SectionLabel } from "./arena-shell-pp1dsK4G.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/matches-Ch666InA.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_jsx_runtime = require_jsx_runtime();
function StatusChip({ status }) {
	const config = {
		completed: {
			style: "border-primary/50 text-primary bg-primary/8 shadow-[0_0_8px_-4px_var(--color-primary)]",
			dot: "bg-primary",
			label: "Completed"
		},
		cancelled: {
			style: "border-destructive/50 text-destructive bg-destructive/8",
			dot: "bg-destructive",
			label: "Cancelled"
		},
		ongoing: {
			style: "border-accent/50 text-accent bg-accent/8 animate-pulse",
			dot: "bg-accent",
			label: "Live"
		}
	}[status] ?? {
		style: "border-border/60 text-muted-foreground",
		dot: "bg-muted-foreground",
		label: status
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
		className: `clip-blade flex items-center gap-1.5 border px-3 py-1 font-mono text-[10px] uppercase tracking-[0.2em] ${config.style}`,
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: `h-1.5 w-1.5 rounded-full flex-shrink-0 ${config.dot}` }), config.label]
	});
}
var MODE_ICON = {
	"1v1": "",
	"four-player": ""
};
function MatchesPage() {
	const { user, loading } = useAuth();
	const navigate = useNavigate();
	(0, import_react.useEffect)(() => {
		if (!loading && !user) navigate({ to: "/login" });
	}, [
		loading,
		user,
		navigate
	]);
	const { data, isLoading, error } = useQuery({
		queryKey: ["matches", user?._id],
		queryFn: api.myMatches,
		enabled: Boolean(user)
	});
	const completed = data?.filter((m) => m.status === "completed").length ?? 0;
	const wins = data?.filter((m) => m.result?.winnerId === user?._id).length ?? 0;
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(ArenaShell, {
		eyebrow: "Combat Log",
		title: "Match history",
		subtitle: "Your last 20 engagements across every region. Click any record to view the full dossier.",
		children: [
			data && data.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "mb-6 grid grid-cols-3 gap-3 animate-rise",
				style: { animationDelay: "0s" },
				children: [
					{
						label: "Total Matches",
						value: data.length
					},
					{
						label: "Completed",
						value: completed
					},
					{
						label: "Wins",
						value: wins,
						accent: true
					}
				].map(({ label, value, accent }) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "clip-blade border border-white/6 bg-surface/30 p-4 text-center backdrop-blur",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: `font-mono text-2xl font-bold tabular-nums ${accent ? "text-accent shadow-[0_0_12px_-4px_var(--color-ember)]" : "text-primary"}`,
						children: value
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "mt-1 font-mono text-[9px] uppercase tracking-widest text-muted-foreground",
						children: label
					})]
				}, label))
			}),
			error && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "mb-5",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Alert, { children: error.message })
			}),
			isLoading && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "flex flex-col gap-3",
				children: [...Array(4)].map((_, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "clip-blade border border-white/5 bg-surface/20 p-6 animate-pulse",
					style: {
						animationDelay: `${i * .1}s`,
						opacity: 1 - i * .15
					},
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "flex items-center gap-3",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "h-4 w-20 rounded bg-white/5" }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "h-4 w-16 rounded bg-white/5" }),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "ml-auto h-3 w-28 rounded bg-white/5" })
						]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "mt-4 flex gap-2",
						children: [...Array(4)].map((_, j) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "h-6 w-20 rounded bg-white/5" }, j))
					})]
				}, i))
			}),
			data && data.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Panel, {
				padding: "p-8",
				className: "shadow-[0_8px_32px_-16px_rgba(0,0,0,0.8)]",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex flex-col items-center gap-4 py-20 text-center px-8",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "font-display text-lg font-bold uppercase tracking-widest text-foreground",
						children: "No engagements yet"
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "mt-2 font-mono text-[11px] uppercase tracking-widest text-muted-foreground",
						children: "Your combat history will appear here"
					})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Link, {
						to: "/play",
						className: "clip-blade bg-primary px-6 py-3 font-display text-xs font-bold uppercase tracking-widest text-primary-foreground shadow-[0_0_20px_-6px_var(--color-primary)] transition-all hover:scale-105 hover:shadow-[0_0_28px_-4px_var(--color-primary)]",
						children: "Enter the Queue"
					})]
				})
			}),
			data && data.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "flex flex-col gap-3",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(SectionLabel, { children: [
					"Recent Engagements · ",
					data.length,
					" records"
				] }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "flex flex-col gap-3 mt-2",
					children: data.map((m, idx) => {
						const isWinner = m.result?.winnerId === user?._id;
						m.players.find((p) => p.userId === user?._id);
						const mmrChange = m.result?.mmrChanges?.[user?._id ?? ""];
						return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Link, {
							to: "/match/$matchId",
							params: { matchId: m.matchId },
							className: "group relative clip-blade overflow-hidden border border-border bg-surface/30 backdrop-blur-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:bg-surface/50 hover:shadow-[0_8px_32px_-12px_rgba(0,0,0,0.6)]",
							style: { animationDelay: `${idx * .05}s` },
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: `absolute left-0 inset-y-0 w-0.5 transition-all ${isWinner ? "bg-accent shadow-[1px_0_8px_var(--color-ember)]" : m.status === "completed" ? "bg-destructive/60" : m.status === "ongoing" ? "bg-primary animate-pulse" : "bg-border/40"}` }),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "pointer-events-none absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity bg-gradient-to-r from-primary/3 to-transparent" }),
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "px-6 py-5",
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										className: "flex flex-wrap items-center justify-between gap-3",
										children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											className: "flex items-center gap-3",
											children: [
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
													className: "flex h-9 w-9 items-center justify-center clip-blade bg-surface/60 text-lg ring-1 ring-border",
													children: MODE_ICON[m.gameMode] ?? "🎮"
												}),
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(StatusChip, { status: m.status }) }),
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
													className: "font-display text-sm font-bold uppercase tracking-[0.18em] text-foreground",
													children: m.gameMode
												}),
												isWinner && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
													className: "clip-blade border border-accent/40 bg-accent/10 px-2 py-0.5 font-mono text-[9px] uppercase tracking-widest text-accent",
													children: "★ Win"
												})
											]
										}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
											className: "flex items-center gap-3",
											children: [
												mmrChange !== void 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
													className: `font-mono text-sm font-bold tabular-nums ${mmrChange > 0 ? "text-primary" : mmrChange < 0 ? "text-destructive" : "text-muted-foreground"}`,
													children: [mmrChange > 0 ? `+${mmrChange}` : mmrChange, " MMR"]
												}),
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
													className: "font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground",
													children: new Date(m.createdAt).toLocaleString()
												}),
												/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
													className: "font-mono text-[9px] text-primary opacity-0 group-hover:opacity-100 transition-opacity",
													children: "View →"
												})
											]
										})]
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
										className: "mt-4 flex flex-wrap gap-2",
										children: m.players.map((p) => {
											const isWinnerPlayer = m.result?.winnerId === p.userId;
											const isMe = p.userId === user?._id;
											return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
												className: `clip-blade border px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.18em] transition-all ${isWinnerPlayer ? "border-accent/60 bg-accent/10 text-accent" : isMe ? "border-primary/40 bg-primary/8 text-primary" : "border-border/50 text-muted-foreground"}`,
												children: [
													isMe ? "YOU" : p.username,
													p.username.startsWith("bot_") && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
														className: "ml-1 opacity-50",
														children: "·bot"
													}),
													" ",
													/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
														className: "opacity-60",
														children: ["· ", p.mmrAtMatch ?? "—"]
													})
												]
											}, p.userId);
										})
									})]
								})
							]
						}, m.matchId);
					})
				})]
			})
		]
	});
}
//#endregion
export { StatusChip, MatchesPage as component };
