...
function createErrorCollectorHooks(options = {}) {
	...
	let outputPath = (0, node_path.resolve)(root, outputFile);
	return {
		configResolved: (config) => {
			root = config.root;
			outputPath = (0, node_path.resolve)(root, outputFile);
		},
		configureServer: (server) => {
			Promise.resolve(server.watcher.unwatch(outputPath)).catch(() => {});
			const initialize = (0, node_fs_promises.mkdir)((0, node_path.dirname)(outputPath), { recursive: true }).then(async () => {
				if (resetOnStart) await (0, node_fs_promises.writeFile)(outputPath, "", "utf8");
			});
			initialize.catch(() => {});
			...
			server.middlewares.use((req, res, next) => {
				...
				readJsonBody$1(req, maxBodyBytes).then(async (parsed) => {
					...
					const write = writeQueue.then(async () => {
						await initialize;
						await (0, node_fs_promises.appendFile)(outputPath, line, "utf8");
					});
...
//#endregion
exports.devServerBridgePlugin = devServerBridgePlugin;
exports.errorCollectorPlugin = errorCollectorPlugin;
