...
export function StatusChip({ status }: { status: Match["status"] }) {
  ...
  return (
    <span className={`clip-blade border px-3 py-1 font-mono text-[10px] uppercase tracking-[0.2em] ${tone}`}>
      {status}
    </span>
  );
}
      ...
      <div className="grid gap-4">
        {data?.map((m) => (
          <Link
            ...
            className="group clip-blade panel p-6 transition-all hover:-translate-y-1 hover:border-primary/50"
          >
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {m.players.map((p) => (
                <span
                  key={p.userId}
                  className={`clip-blade border px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.18em] ${
