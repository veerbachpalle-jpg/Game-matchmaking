...
import type { ArrayKey, IsTuple, Prev, TupleKeys } from '../../../src/types/path/common';
/**
 ...
 * See {@link Path}
 */
type PathImpl<K extends string | number, V, TraversedTypes, D extends number = 9> = V extends Primitive | BrowserNativeObject ? `${K}` : true extends AnyIsEqual<TraversedTypes, V> ? `${K}` : `${K}` | `${K}.${PathInternal<V, TraversedTypes | V, Prev[D]>}`;
/**
 * Helper type for recursively constructing paths through a type.
...
type PathInternal<T, TraversedTypes = T, D extends number = 9> = [D] extends [
    never
] ? never : T extends ReadonlyArray<infer V> ? IsTuple<T> extends true ? {
    [K in TupleKeys<T>]-?: PathImpl<K & string, T[K], TraversedTypes, D>;
}[TupleKeys<T>] : PathImpl<ArrayKey, V, TraversedTypes, D> : {
...
type ArrayPathInternal<T, TraversedTypes = T, D extends number = 9> = [
    D
] extends [never] ? never : T extends ReadonlyArray<infer V> ? IsTuple<T> extends true ? {
    [K in TupleKeys<T>]-?: ArrayPathImpl<K & string, T[K], TraversedTypes, D>;
}[TupleKeys<T>] : ArrayPathImpl<ArrayKey, V, TraversedTypes, D> : {
